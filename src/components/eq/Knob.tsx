...
export function Knob({ label, size, defaultValue = 0, onChange, engineActive = false }: KnobProps) {
    ...
    const ticks = TICKS.map(({ db, kind }) => {
        ...
        const innerR =
            kind === "zero" ? tickInnerZero : kind === "major" ? tickInnerMajor : tickInnerMinor;
        return {
            db,
            kind,
    ...
    const arcStroke = engineActive ? `hsl(87,90%,${arcL.toFixed(1)}%)` : "#50534d";
    const arcGlowOpacity = engineActive ? (0.2 + 0.3 * arcT).toFixed(2) : "0";

    // dB readout: "0.0 dB", "+2.4 dB", "-3.6 dB"
    const isZero = Math.abs(defaultValue) < 0.05;
    const dbText = isZero
        ? "0.0 dB"
        : `${defaultValue > 0 ? "+" : ""}${defaultValue.toFixed(1)} dB`;
    ...
    const handlePointerDown = useCallback(
        (e: React.PointerEvent) => {
            ...
            const move = (ev: PointerEvent) => {
                const nv = Math.max(
                    MIN_DB,
                    Math.min(MAX_DB, startVal + (startY - ev.clientY) * 0.15)
                );
                onChange?.(nv);
            };
            const up = () => {
    ...
    return (
        <div style={{ textAlign: "center" }}>
            <div
                ...
                <svg
                    ...
                    <circle
                        ...
                        style={{
                            filter: `drop-shadow(0 0 5px rgba(132,232,12,${arcGlowOpacity}))`
                        }}
                    />
                    {/* Tick marks */}
                ...
                <div
                    style={{
                        ...
                        width: bodySize,
                        height: bodySize,
                        marginLeft: -(bodySize / 2),
                        marginTop: -(bodySize / 2),
                        borderRadius: "50%",
                        background:
                            "radial-gradient(circle at 36% 30%,#34363d 0%,#1a1b1f 62%,#141519 100%)",
                ...
                <div
                    style={{
                        ...
                        height: needleH,
                        borderRadius: "2px",
                        background: engineActive ? ACCENT : "#5a5d57",
                        boxShadow: engineActive ? "0 0 12px rgba(132,232,12,.8)" : "none",
            ...
            <div
                style={{
                    fontSize: "10px",
                    color: isZero ? "#5d5d65" : engineActive ? ACCENT : "#687044",
                    marginTop: "3px",
